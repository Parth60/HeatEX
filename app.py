...
from backend.app.core.release import VERSION, RELEASE_NAME, release_manifest
from backend.app.core.units import UNIT_SYSTEMS, build_display_summary
...
from backend.app.core.diagnostics import environment_diagnostics
from backend.app.visualization.release_ui import final_release_css, release_banner_html
...
st.markdown(final_release_css(), unsafe_allow_html=True)
...
with st.sidebar:
    st.markdown("### PROJECT GARAGE")

    project_name_v100 = st.text_input(
        "Project name",
        value=st.session_state.get("project_name_v100", "HeatEX Design Case"),
        key="project_name_v100",
    )

    engineer_v100 = st.text_input(
        "Engineer / student",
        value=st.session_state.get("engineer_v100", ""),
        key="engineer_v100",
    )

    display_units_v100 = st.selectbox(
        "Display units",
        UNIT_SYSTEMS,
        index=UNIT_SYSTEMS.index(
            st.session_state.get("display_units_v100", "SI Engineering")
            if st.session_state.get("display_units_v100", "SI Engineering") in UNIT_SYSTEMS
            else "SI Engineering"
        ),
        key="display_units_v100",
        help="The solver remains SI internally. This changes the final project summary display only.",
    )

    preset_choice_v100 = st.selectbox(
        "Quick-start preset",
        preset_names(),
        key="preset_choice_v100",
    )

    pset = get_preset(preset_choice_v100)
    st.caption(pset["description"])

    if st.button(
        "APPLY PRESET",
        use_container_width=True,
        key="apply_preset_v100",
    ):
        for key, value in pset["ui_state"].items():
            st.session_state[key] = value

        # Clear data-editor state when moving back to pure-fluid presets.
        st.session_state.pop("hot_composition_editor", None)
        st.session_state.pop("cold_composition_editor", None)
        st.rerun()

    project_upload_v100 = st.file_uploader(
        "Restore HX//RACE project",
        type=["json"],
        key="project_upload_v100",
        help="Load a project snapshot generated by HX//RACE v1.0.",
    )

    if project_upload_v100 is not None:
        if st.button(
            "LOAD PROJECT",
            use_container_width=True,
            key="load_project_v100",
        ):
            try:
                loaded_project_v100 = parse_project_bytes(
                    project_upload_v100.getvalue()
                )

                for key, value in loaded_project_v100["ui_state"].items():
                    st.session_state[key] = value

                metadata = loaded_project_v100.get("project", {})
                st.session_state["project_name_v100"] = metadata.get(
                    "name", "HeatEX Design Case"
                )
                st.session_state["engineer_v100"] = metadata.get("engineer", "")
                units = metadata.get("display_units", "SI Engineering")
                if units in UNIT_SYSTEMS:
                    st.session_state["display_units_v100"] = units

                st.session_state["loaded_project_record_v100"] = loaded_project_v100

                # Re-seed composition editors from the stored lists.
                st.session_state.pop("hot_composition_editor", None)
                st.session_state.pop("cold_composition_editor", None)

                st.success("Project loaded. Recalculating with restored inputs…")
                st.rerun()
    ...
    with st.expander("Quick start"):
        st.write(
            "1. Pick a preset or restore a project. "
            "2. Review MACHINE SETUP. "
            "3. Enter streams in LIVE RUN. "
            "4. Review GEOMETRY. "
            "5. Check VALIDATION. "
            "6. Export the TECH PACK."
        )
    ...
    mode = st.radio(
        "Mode",
        ["Design", "Rating preview"],
        horizontal=True,
        key="mode_v100",
        help=(
            "Design is the primary v1.0 workflow. Rating preview retains the same "
            "target-outlet calculation path and is not an independent installed-area rating solver."
        ),
    )
    ...
    thermo_package = st.selectbox(
        ...
        key="thermo_package_v100",
        help=(
            "HX//RACE uses enthalpy-based stream balances. PR/SRK add cubic-EOS "
...
tab_run, tab_thermo, tab_flash, tab_multi, tab_alternatives, tab_phase, tab_geometry, tab_mechanical, tab_bell, tab_optimizer, tab_validation, tab_project, tab_export = st.tabs(
    [
        "🏁 LIVE RUN",
        "🧪 THERMODYNAMICS",
        "⚗️ FLASH / VLE",
...
with tab_geometry:
    ...
    with g1:
        shell_id = st.number_input(
            "Shell ID [m]", min_value=0.15, value=0.80, step=0.01,
            key="shell_id_v100",
        )
        tube_od_mm = st.number_input(
            "Tube OD [mm]", min_value=6.0, value=19.05, step=0.1,
            key="tube_od_mm_v100",
        )

    with g2:
        tube_id_mm = st.number_input(
            "Tube ID [mm]", min_value=4.0, value=15.75, step=0.1,
            key="tube_id_mm_v100",
        )
        tube_length = st.number_input(
            "Tube length [m]", min_value=0.5, value=6.0, step=0.1,
            key="tube_length_v100",
        )

    with g3:
        tube_count = st.number_input(
            "Tube count", min_value=4, value=420, step=1,
            key="tube_count_v100",
        )
        tube_passes = st.selectbox(
            "Tube passes", [1, 2, 4, 6, 8], index=1,
            key="tube_passes_v100",
        )

    with g4:
        tube_pitch_mm = st.number_input(
            "Tube pitch [mm]", min_value=8.0, value=25.0, step=0.1,
            key="tube_pitch_mm_v100",
        )
        tube_layout = st.selectbox(
            "Tube layout",
            ["triangular", "square", "rotated square"],
            key="tube_layout_v100",
        )

    b1, b2 = st.columns(2)

    with b1:
        baffle_count = st.slider(
            "Number of baffles", 0, 30, 12, 1,
            key="baffle_count_v100",
        )

    with b2:
        baffle_cut = st.slider(
            "Baffle cut [%]", 10, 50, 25, 1,
            key="baffle_cut_v100",
        )
...
# v1.0: duplicate incremental UI blocks removed.
...
if sim_error:
    ...
    # ---------------------------------------------------------
    # v1.0 project snapshot and release dashboard
    # ---------------------------------------------------------
    ui_state_v100 = {
        "mode_v100": mode,
        "front_head_v100": front,
        "shell_type_v100": shell,
        "rear_head_v100": rear,
        "thermo_package_v100": thermo_package,
        "flow_arrangement_v100": flow_arrangement,
        "allowable_tube_dp_v100": float(allowable_tube_dp),
        "allowable_shell_dp_v100": float(allowable_shell_dp),

        "hot_stream_definition": hot_mode,
        "hot_fluid_v100": hot_fluid if hot_mode == "Pure fluid" else "Ethanol",
        "hot_basis": hot_basis,
        "hot_tin_v100": float(hot_tin),
        "hot_p_v100": float(hot_p),
        "hot_flow_v100": float(hot_flow),
        "hot_tout_v100": float(hot_tout),
        "tube_fouling_v100": float(tube_fouling),

        "cold_stream_definition": cold_mode,
        "cold_fluid_v100": cold_fluid if cold_mode == "Pure fluid" else "Water",
        "cold_basis": cold_basis,
        "cold_tin_v100": float(cold_tin),
        "cold_p_v100": float(cold_p),
        "cold_flow_v100": float(cold_flow),
        "shell_fouling_v100": float(shell_fouling),
        "wall_k_v100": float(wall_k),

        "shell_id_v100": float(shell_id),
        "tube_od_mm_v100": float(tube_od_mm),
        "tube_id_mm_v100": float(tube_id_mm),
        "tube_length_v100": float(tube_length),
        "tube_count_v100": int(tube_count),
        "tube_passes_v100": int(tube_passes),
        "tube_pitch_mm_v100": float(tube_pitch_mm),
        "tube_layout_v100": str(tube_layout),
        "baffle_count_v100": int(baffle_count),
        "baffle_cut_v100": int(baffle_cut),

        "mech_shell_material_v100": mech_shell_material,
        "mech_tube_material_v100": mech_tube_material,

        "hot_composition_seed_v100": (
            [{"Component": k, "Fraction": v} for k, v in (hot_composition or {}).items()]
            if hot_mode == "Mixture"
            else []
        ),
        "cold_composition_seed_v100": (
            [{"Component": k, "Fraction": v} for k, v in (cold_composition or {}).items()]
            if cold_mode == "Mixture"
            else []
        ),
    }

    project_snapshot_v100 = build_project_snapshot(
        project_name=project_name_v100,
        engineer=engineer_v100,
        notes=st.session_state.get("project_notes_v100", ""),
        display_units=display_units_v100,
        ui_state=ui_state_v100,
        report_payload=report_payload,
    )

    with tab_project:
        st.markdown("### Project / Release Garage")

        st.markdown(
            f"""
            <div class="hx-release-grid">
              <div class="hx-release-card">
                <div class="hx-release-kicker">RELEASE</div>
                <div class="hx-release-value">v{VERSION}</div>
              </div>
              <div class="hx-release-card">
                <div class="hx-release-kicker">CONFIGURATION</div>
                <div class="hx-release-value">{result.tema_code}</div>
              </div>
              <div class="hx-release-card">
                <div class="hx-release-kicker">READINESS</div>
                <div class="hx-release-value">{validation_report.readiness_score:.1f}/100</div>
              </div>
              <div class="hx-release-card">
                <div class="hx-release-kicker">DISPLAY UNITS</div>
                <div class="hx-release-value">{display_units_v100}</div>
              </div>
            </div>
            """,
            unsafe_allow_html=True,
        ...
        st.markdown("#### Converted project summary")

        display_df_v100 = pd.DataFrame(
        ...
        project_notes_v100 = st.text_area(
            "Project notes",
            value=st.session_state.get("project_notes_v100", ""),
            key="project_notes_v100",
            height=100,
        )

        # Rebuild once after the notes widget so saved notes are current.
        project_snapshot_v100 = build_project_snapshot(
            project_name=project_name_v100,
            engineer=engineer_v100,
            notes=project_notes_v100,
            display_units=display_units_v100,
            ui_state=ui_state_v100,
            report_payload=report_payload,
        )
        ...
        with pc1:
            st.download_button(
                "↓ SAVE HX//RACE PROJECT",
                data=project_to_bytes(project_snapshot_v100),
                file_name=(
                    project_name_v100.strip().replace(" ", "_")
                    or "HX-RACE_Project"
                ) + ".hxrace.json",
                mime="application/json",
                use_container_width=True,
        ...
        with pc2:
            st.download_button(
                "↓ FINAL TECHNICAL PACK",
                data=technical_pack_zip,
                file_name=f"{result.tema_code}_HX-RACE_Technical_Pack_v1.0.zip",
                mime="application/zip",
                use_container_width=True,
            )

        st.markdown("#### Release diagnostics")
        diagnostics_v100 = environment_diagnostics()
        st.json(diagnostics_v100)

        if not diagnostics_v100["optional_packages"]["CoolProp"]:
            st.info(
                "CoolProp is not installed in the core environment. "
                "HX//RACE remains functional using its internal property fallback. "
                "Install requirements-optional.txt when supported."
            )

        manifest_v100 = release_manifest()
        with st.expander("v1.0 release manifest"):
            st.json(manifest_v100)

        st.caption(
            "Project restore recalculates the engineering model from restored inputs. "
            "Saved result data are retained for traceability, not blindly reused as a new result."
        )
    ...
    with tab_validation:
        ...
        with dlv1:
            st.download_button(
                ...
                file_name=f"{result.tema_code}_HX-RACE_Engineering_Report_v1.0.pdf",
    ...
    with tab_export:
        st.markdown("### Technical Pack — v1.0")
...
st.caption(
    "HX//RACE v1.0 · Final Capstone Release · "
...
st.divider()
st.caption(
    "HX//RACE v1.0.0 Final Capstone Release · Calculations remain engineering-screening outputs. "
    "Not TEMA Certified · Not ASME Stamped · Not for fabrication without qualified review."
)
